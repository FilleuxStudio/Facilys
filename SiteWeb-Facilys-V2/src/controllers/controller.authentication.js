...
exports.logout = (req, res) => {
  ...
  res.redirect('/home'); // Rediriger vers la page de connexion
};

// Contrôleur d'inscription
exports.register = async (req, res) => {
    const { _csrf, companyName, firstName, lastName, email, password, passwordControl } = req.body;
    if (!req.csrfToken() || _csrf !== req.csrfToken()) {
    ...
    try {
      ...
      var hashedPassword = await argon2.hash(password);
        ...
        if(checkTerm == true){
         ...
         return res.status(201).redirect('/register?Vous devez accepter les conditions d\'utilisation');
        }
...
function generateRandomProfileImage(size = 128) {
  ...
  const colors = [
    '#f6fdf6', '#d3fdd6', '#83fb91', '#78e5c1', '#24cc8c',
    '#78badb', '#6dade7', '#77a3e1', '#6a8edb', '#7971ce',
    '#a9978a', '#a18884', '#947579', '#896c7c', '#665e83',
    '#7b68b8', '#9f63c1', '#c367b1', '#d47391', '#ea7661'
  ];
